...
HRESULT Image::Init(const char * fileName,
	float x, float y, int width, int height,
	int keyFrameX, int keyFrameY, bool isBackground,
	bool trans, COLORREF transColor)
{
	HDC hdc = GetDC(g_hWnd);
	//int maxSize = width > height ? width : height;
	...
	imageInfo = new IMAGE_INFO;
	//imageInfo->rotateMaxSize = maxSize * 6;
	imageInfo->resID = 0;
	imageInfo->hMemDC = CreateCompatibleDC(hdc);
	imageInfo->hBitmap = (HBITMAP)LoadImage(g_hInstance, fileName,
		IMAGE_BITMAP, width, height, LR_LOADFROMFILE);
...
//씬매니저에서 사용
void Image::FrameRender_Title(HDC hdc, int destX, int destY, int currentKeyFrameX, int currentKeyFrameY, float scale)
{
	// 현재 키프레임 인덱스가 최대 키프레임 인덱스보다 클 때
	imageInfo->currentKeyFrameX = currentKeyFrameX;
	imageInfo->currentKeyFrameY = currentKeyFrameY;
...
void Image::FrameRenderHP(HDC hdc, int destX, int destY,
	int currentKeyFrameX, int currentKeyFrameY, float scale, bool isHp)
{
	// 현재 키프레임 인덱스가 최대 키프레임 인덱스보다 클 때
	imageInfo->currentKeyFrameX = currentKeyFrameX;
	imageInfo->currentKeyFrameY = currentKeyFrameY;
	...
	if (isHp)
	{
		GdiTransparentBlt(
			hdc,
			destX, destY,
			imageInfo->keyFrameWidth * currentKeyFrameX,
			imageInfo->keyFrameHeight,
...
void Image::ScrollRender(HDC hdc, int Mapsize_y)
{
	static int nY = Mapsize_y - 1000;

	if (nY < 0)
	{
		BitBlt(hdc, 0, -nY, 800, 1000 + nY, imageInfo->hMemDC, 0, 0, SRCCOPY);
		BitBlt(hdc, 0, 0, 800, -nY, imageInfo->hMemDC, 0, Mapsize_y + nY, SRCCOPY);
	}
	else
	{
		BitBlt(hdc, 0, 0, 800, 1000, imageInfo->hMemDC, 0, nY, SRCCOPY);
	}
	nY -= 150 * TimeManager::GetSingleton()->GetDeltaTime();
	if (nY + 1000 <= 0) nY += Mapsize_y;
}
...
void Image::RotateAlphaRender(HDC hdc, double dblAngle, int destX, int destY, BYTE alpha, double dblSizeRatio, HBITMAP hMaskBmp, int ixMask, int iyMask)
{
	...
	if (isTrans)
	{
		...
		AlphaBlend(hdc,
			...
			imageInfo->rotateMaxSize, imageInfo->rotateMaxSize,
			imageInfo->hBlendDC, 0, 0, imageInfo->rotateMaxSize, imageInfo->rotateMaxSize,
			blendFunc);
	}
	else
	{
...
void Image::FrameRender_Ending(HDC hdc, int destX, int destY, int currentKeyFrameX, int currentKeyFrameY, float scale)
{
	// 현재 키프레임 인덱스가 최대 키프레임 인덱스보다 클 때
	imageInfo->currentKeyFrameX = currentKeyFrameX;
	imageInfo->currentKeyFrameY = currentKeyFrameY;
